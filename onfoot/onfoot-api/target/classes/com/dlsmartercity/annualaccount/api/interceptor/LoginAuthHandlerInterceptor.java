...
public class LoginAuthHandlerInterceptor extends HandlerInterceptorAdapter {
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
		...
		if(StringUtils.isEmpty(tokenid)) {
			BaseResponse appResponse = new BaseResponse();
			appResponse.setRetcode(RetMessageKey.RESPONSE_CODE_TOCKENID_MISSING);
			appResponse.setRetmessage(MessageSourceHolder.getMessage(RetMessageKey.RESPONSE_CODE_TOCKENID_MISSING));
			appResponse.setTokenid(tokenid);
			response.setContentType(MediaType.APPLICATION_JSON_UTF8_VALUE);
			response.getWriter().write(JsonUtils.serialize(appResponse));
			response.getWriter().flush();
